...
class Product(models.Model):
    ...
    status = models.CharField(
        max_length=10, choices=APP_VALUE_STATUS_CHOICE, default='draft')
    ordering = models. IntegerField(default=0)
    special = CustomBooleanField()
    # give us decimal numbers: 10 digit with 0 decimal place
    price = models.DecimalField(max_digits=10, decimal_places=0)
    price_sale = models.DecimalField(
        max_digits=10, decimal_places=0, null=True, blank=True)  # sale price
    # hide from the admin page, cannot be edited
    ...
    planting_methods = models.ManyToManyField(
        PlantingMethod)  # many-to-many relationships
